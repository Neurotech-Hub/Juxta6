...
The BME688 and ADXL367 share the same bus. Confirmed pin assignment:
...
The user guide lists the following exposed GPIOs. **Custom Function** is the Juxta harness assignment on this Tag: BTN2 and LED2 remain unpopulated/unused, so their footprints are available for external wiring. Keep shared TWI and project QSPI CS. Do **not** remap U8/`spi00` onto these pads for SD — use a separate SPI instance (e.g. SPIM20/21) in SPI SD mode.


| GPIO    | Default board use                      | Project interpretation                         | Custom Function              |
| ------- | -------------------------------------- | ---------------------------------------------- | ---------------------------- |
| `P2.07` | LED2 blue footprint                    | Available (LED2 unused)                        | SD MISO                      |
| `P1.02` | None                                   | Custom harness                                 | SD SCK                       |
| `P1.03` | None                                   | Custom harness                                 | SD MOSI                      |
| `P0.01` | BTN2 footprint                         | Available (BTN2 unused)                        | SD CS                        |
| `P2.05` | QSPI CS when external flash is mounted | **Reserved in this project** (U8 fitted)       | —                            |
| `P2.06` | None                                   | Custom harness                                 | SD detect                    |
| `P1.11` | TWI SCL                                | **Reserved** — maintain shared TWI             | TWI SCL                      |
| `P1.12` | TWI SDA                                | **Reserved** — maintain shared TWI             | TWI SDA                      |
| `P1.13` | None                                   | Custom harness                                 | Power good                   |
| `P1.14` | None                                   | Custom harness                                 | RTC interrupt                |
| `P0.02` | LED2 red footprint                     | Available (LED2 unused)                        | LED                          |
| `P0.04` | LED2 green footprint                   | Available (LED2 unused)                        | PIR sensor input             |
...
- **Required:** TWI (`P1.11`/`P1.12`), SD SPI (`P2.07` MISO / `P1.02` SCK / `P1.03` MOSI / `P0.01` CS), PIR (`P0.04`).
- Also assigned: RTC interrupt (`P1.14`), status LED (`P0.02`), SD card detect (`P2.06`), power-good (`P1.13`).
- SD is SPI-mode only on this pin budget; level-shift / power the card independently of Tag `VDD` if required.
