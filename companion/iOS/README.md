...
iPhone companion for **Juxta6-0** Tags (`juxta6-0-prod`, firmware **6.5.0**, schema **`jxta-nor-csv-v9`**). Connect over Bluetooth Low Energy (Hublink), sync session settings, transfer daily CSV packages, and inspect plots offline.
...
- Tag running Juxta6 firmware **6.x** (current: `6.5.0`)
- Location When In Use (optional) — used for gateway lat/lon; Open-Meteo outdoor °C is display-only on the clock

## Features

- **BLE scan & connect** — Hublink service UUID; peripherals advertise as `JX_*` (Mobile) or `JB_*` (Base)
...
- **Device settings** — subject, experiment, Mobile Tag / Base Station role, adv/scan interval (0 = off, else 1–120 s), vitals ≥60 s, inactivity multiplier (1–10), motion logging. Changing role clears on-device memory and renames the ADV identity (`JX_` ↔ `JB_`)
...
`peer_id` is `X` (mobile) or `B` (base) + 6 hex. The companion reconstructs `JX_`/`JB_` labels in plots. Observer column is omitted (device id is in the package folder).
...
UTC `timestamp`, `sendFilenames`, optional `latitude` / `longitude`, settings fields including `isBaseStation`, `clearMemory`, `reset` (shelf). Adv/scan intervals clamped to 0 or 1–120; vitals interval minimum 60 s. Role changes should include `clearMemory: true` (firmware also enforces erase).
