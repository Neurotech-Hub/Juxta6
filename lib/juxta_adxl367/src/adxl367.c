...
/*
 * Juxta vendored copy of the Zephyr ADXL367 driver
 * (zephyr/drivers/sensor/adi/adxl367/, NCS v3.3.4). Built in place of the
 * in-tree driver (CONFIG_ADXL367=n). Two deliberate changes:
 *
 * 1. adxl367_probe() no longer runs adxl367_self_test(). The self-test
 *    compares two single X-axis samples taken ~320 ms apart against a
 *    90-270 mg window; any handling motion (battery insertion, magnet
 *    reset in hand) corrupts it and intermittently fails boot. The soft
 *    reset + DEVID/PARTID checks remain, so a dead/absent chip still
 *    fails probe with -ENODEV.
 * 2. CONFIG_ADXL367_* Kconfig symbols in the device config macro are
 *    replaced with their upstream defaults (the in-tree Kconfig menu
 *    disappears with CONFIG_ADXL367=n). Trigger/stream/async paths are
 *    unused here: juxta_motion polls via the standard sensor API.
 */
...
static int adxl367_probe(const struct device *dev)
{
	...
	/*
	 * Juxta: adxl367_self_test() intentionally NOT run here. It requires a
	 * motionless device (two single X-axis samples, 90-270 mg window) and
	 * these devices boot in the user's hand. See file header.
	 */
...
#define ADXL367_CONFIG(inst, chipid)							\
		...
		/* Juxta: upstream Kconfig defaults, hardcoded (CONFIG_ADXL367=n). */	\
		.activity_th.value = 100,						\
		.activity_th.referenced = true,						\
		.activity_th.enable = true,						\
		.activity_time = 100,							\
		.inactivity_th.value = 100,						\
		.inactivity_th.referenced = true,					\
		.inactivity_th.enable = true,						\
		.inactivity_time = 100,							\
