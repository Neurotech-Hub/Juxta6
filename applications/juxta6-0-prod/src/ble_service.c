...
static atomic_t datetime_synced;
...
{
	return atomic_get(&datetime_synced) != 0;
}

void juxta_ble_clear_datetime_synced(void)
{
	atomic_set(&datetime_synced, 0);
}

/* LiPo curve kept for reference companions; Tag uses CR2032 estimate. */
static uint8_t batt_mv_to_percent(int32_t mv)
{
	...
	return juxta_vdd_mv_to_percent_cr2032(mv);
}
...
static int apply_gateway_command(const char *json)
{
	...
	else if (extract_u32(json, "timestamp", &value) == 0)
	{
		...
		atomic_set(&datetime_synced, 1);
		juxta_rtt_jxs("time_set");
	...
	if (changed)
	{
		...
		if (production_ready)
		{
			(void)juxta_log_append_event(log_ctx, a, device_id, "settings_changed",
						     juxta_time_now());
			juxta_rtt_jxs("settings_changed");
		}
